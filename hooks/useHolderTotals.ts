...
export function useHolderTotals(assets: AssetEntry[] | null) {
  ...
  return useAsync<number>(
    async () => {
      ...
      const metas = await mapWithConcurrency(assets, DEFAULT_CONCURRENCY, (a) =>
        assetToken.getMetadata(network, a.tokenContract),
      );
      const complianceContracts = Array.from(
        new Set(metas.map((m) => m.complianceContract)),
      );
      const lists = await mapWithConcurrency(complianceContracts, DEFAULT_CONCURRENCY, (c) =>
        compliance.getAllowlist(network, c),
      );
