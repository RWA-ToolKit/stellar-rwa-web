...
/** False when a network's contract ids haven't been configured (e.g. mainnet pre-deploy). */
export function isNetworkConfigured(network: Network): boolean {
  const ids = IDS[network];
  return Boolean(ids.registry && ids.compliance && ids.dividend);
}
...
export const assetToken = {
  ...
  pause(ctx: WriteCtx, tokenId: string): Promise<TxResult> {
    return write(ctx, tokenId, "pause", [arg.address(ctx.source)], "asset-token");
  },

  unpause(ctx: WriteCtx, tokenId: string): Promise<TxResult> {
    return write(ctx, tokenId, "unpause", [arg.address(ctx.source)], "asset-token");
  },
};
